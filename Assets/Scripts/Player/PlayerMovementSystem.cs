...
public class PlayerMovementSystem : ReactiveSystem<InputEntity> 
{
	...
	protected override void Execute(List<InputEntity> entities) 
	{
		foreach (var e in entities)
		{
			...
			x += e.input.moveDirection.x * Time.deltaTime * GameConfig.Instance.playerSpeed;
			y += e.input.moveDirection.y * Time.deltaTime * GameConfig.Instance.playerSpeed;
			
			_contexts.game.playerEntity.ReplacePosition(x,y,z);

			if (e.input.moveDirection.x != 0)
			{
				_contexts.game.playerEntity.ReplaceDirection(e.input.moveDirection.x > 0 ? Direction.Right : Direction.Left);
			}
